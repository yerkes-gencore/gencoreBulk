...
#' \dontrun{
#'  ## Simple call
#'   heatmapFromGenelist(geneList = c('Ccl2', 'Cxcl1', 'Cxcl2', 'Postn', 'Fn1', 'Thbs1'),
...
heatmapFromGenelist <- function(geneList,
  ...
  ComplexHeatmap::Heatmap(hmap,
                          ...
                          top_annotation = (if (!is.null(slice_labels)){
                            if (is.null(column_split)){warning('Setting labels requires slices to also be set')}
                            HeatmapAnnotation(foo = anno_block(gp = gpar(col=NA),
                                                               labels = slice_labels, 
